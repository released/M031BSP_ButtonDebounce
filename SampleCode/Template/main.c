...
uint32_t button1_tick = 0;
uint32_t button2_tick = 0;
...
{
	if (PA6 == FALSE)	// button1 pull high , and keep pressed button
	{
		if (((get_tick() - button1_tick) % 1000) == 0)
		{
    		printf("PA6 button pressed : %4d\r\n", get_tick());
		}
	}
	else
	{
	...
	if (PA7 == TRUE)	// button2 pull low , and keep pressed button
	{
		if (((get_tick() - button2_tick) % 1000) == 0)
		{
    		printf("PA7 button pressed : %4d\r\n", get_tick());
		}
	}
	else
	{
...
void Button_Init (void)
{
    ...
    GPIO_EnableInt(PA, 7, GPIO_INT_RISING);
